...
if ($_GET["Command"] == "getdt") {
    ...
    $sql = "SELECT faudcode FROM invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    //$no = $row['faudcode'];
    $post = generateId($row['faudcode'], "UD/", "post");
    $uniq = uniqid();
...
if ($_GET["Command"] == "save_item") {
    ...
    try {
        ...
        $sql = "delete from userdept where refno = '" . $_GET['refno'] . "'";
        $result = $conn->query($sql);
        $sql = "Insert into userdept(refno,uniq,userdep)values 
           ('" . $_GET['refno'] . "','" . $_GET['uniq'] . "','" . $_GET['userdept_txt'] . "')";
        ...
        $no = $row['faudcode'];
        $no2 = $no + 1;
        $sql = "update invpara set faudcode = $no2 where faudcode = $no";
        $result = $conn->query($sql);
...
if ($_GET["Command"] == "delete") {
    ...
    $sql = "update userdept set cancel = '1'   where refno = '" . $_GET['refno'] . "'";
    $result = $conn->query($sql);
